...
AS SELECT 
    ...
    ROW_NUMBER() OVER (PARTITION BY fts_month, fts_year, inn, company_name, country_of_departure, 
        code_gng, goods_name, port_terminal
    ) AS count_unique_containers_fts,
    if(count_unique_containers_fts = 1, count_containers_fts, NULL) AS containers_fts_by_row,
    ROW_NUMBER() OVER (PARTITION BY fts_month, fts_year, code_gng, ktk.destination_region, ktk.rf_destination_station,
...
INNER JOIN (SELECT departure_month, departure_year,
			...
			GROUP BY departure_month, departure_year, cargo_code_gng,
