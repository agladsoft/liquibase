...
FROM (SELECT month_parsed_on, year_parsed_on, shipment_month, shipment_year, terminal, line_unified, ship_name_unified, voyage, shipment_date, consignment, min_consignment, direction, container_number, count_dupl_cont_num, container_size, container_type_unified, teu, count, 1 AS count_main, is_empty, goods_name, tnved, tnved_group_name, goods_weight_netto, goods_weight_brutto, shipper_name, consignee_name, consignee_inn, consignee_name_unified, city, tracking_country_unified, tracking_seaport_unified, region, lat_port, long_port
FROM (
